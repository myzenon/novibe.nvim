...
> **A — Hybrid**: novibe format is added to `AGENTS.md` so opencode always knows the `.no_vibe/` structure, even in interactive sessions and agent tasks outside of novibe fills. Best when you already use `AGENTS.md` for project conventions or want the whole team to benefit.
>
...
Skip the instruction file entirely. The knowledge base built in Step 2 is the full source of truth for novibe. If a team `AGENTS.md` already exists in the repo, opencode will still read it for base behavior — novibe's KB adds on top at fill time.

...
2. "When you use plan mode (e.g. `/plan` in Claude Code, or ask any AI to make a plan before coding), do you want that plan saved to task.md so you can continue it on another machine or tool?"
...
3. Append this block to `CLAUDE.md` (create it if missing):

   ```markdown
   ## Session state
   At the start of each session, if `.no_vibe/task.md` exists, read it and acknowledge the current task state in one sentence.
   When using plan mode or creating a multi-step plan, write the plan to `.no_vibe/task.md` immediately so it survives across sessions and tools.

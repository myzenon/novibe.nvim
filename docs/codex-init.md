...
You can also grow the knowledge base lazily during `:NovibeConsult` sessions: the seed context is passed as the initial prompt when Codex starts. Say **"snapshot"** whenever you discover something worth keeping, and write it to the right file with the current commit hash.
...
- Profiles are defined in your `setup()` call with `provider = "codex"` and `model`. The `effort` field is ignored for Codex (no CLI equivalent). No active profile = CLI defaults.
- Use a fast model (e.g. `o4-mini`) for routine fills and a powerful one (e.g. `o3`) for complex consult sessions.
...
- Codex fills are non-streaming — the fill-preview split appears complete when the response arrives, rather than streaming in progressively
- Session continuity is automatic: novibe reuses the `thread_id` from each response for follow-up fills; `:NovibeReset` starts a fresh session

...
```
#teach diffs  →  learned-*.md (staged)  →  convention-*.md (canonical)
...
Captured evidence accumulates in `.no_vibe/diffs.json` (don't edit this; add it to `.gitignore`). When enough evidence accumulates, novibe automatically distills it into `learned-*.md` topic files.

### Auto-distill threshold

| Project state | Threshold |
|---|---|
| Fresh (no `learned-*.md` yet) | **1** — first `#teach` triggers distillation immediately |
...
The prompt sends all accumulated diffs to the AI alongside any existing `learned-*.md` files. The AI:
...
4. Splits output into topic files: `learned-style.md`, `learned-react.md`, etc.
...
**Do not edit `learned-*.md` by hand** — future distillations regenerate and merge them, overwriting your edits.

---

## The staging area

`learned-*.md` files are the AI's working hypothesis about your style:
...
- **Browsable** via `:NovibeKB` → Learn

Think of `learned-*.md` as "rules the AI believes but hasn't confirmed with you yet." You confirm them by promoting.

---

## Promotion

Promotion graduates a rule from staging (`learned-*.md`) to canonical (`convention-*.md`).

| File | Role |
|---|---|
| `learned-*.md` | Mutable staging. AI's hypothesis. Regenerated by distillation. |
| `convention-*.md` | Canonical. Committed rules. Distillation never touches them. |
...
The plugin reads all `learned-*.md` and existing `convention-*.md` files, asks the model to identify mature rules and propose where they should land, then opens the chat review panel:

```
[1/3] .no_vibe/convention-style.md  [create]
...
| `"put style rules in convention-me.md"` + `:w` | AI revises the destination |
| `"also promote no semicolons"` + `:w` | AI adds the rule |
| `q` | Cancel — nothing is written |

**Destination filename is your call.** `convention-style.md` for shared team rules, `convention-me.md` for personal preferences (add that to `.gitignore`).

After promotion, the rule loses its `<!-- n=N -->` annotation — conventions are canonical, the count is no longer needed. The rule is also removed from `learned-*.md`. Future distillations won't re-add it because the model sees the convention files as context.
...
3. gv → :NovibeAct → #teach prefer X over Y
        ...
        .no_vibe/learned-style.md
        - prefer X over Y  <!-- n=1 -->
                ↓  (more diffs reinforce the rule)
        .no_vibe/learned-style.md
        - prefer X over Y  <!-- n=4 -->
                ↓  (:NovibePromote, n >= 3)
        .no_vibe/convention-style.md
...
If you already know the rule and it's firm ("always use named exports"), write it in `convention-*.md` directly. Use `#teach` for preferences you're still observing — let evidence accumulate before committing.
...
Put personal preferences in `convention-me.md` and gitignore it. The plugin loads all matching convention files; teammates get the shared ones, you get yours too.

...
describe("learn.teach — diff capture", function()
  ...
  it("saves a note (no original) when there is no diff but a reason is given", function()
    with_tmp_cwd(function(dir)
      learn.teach("same", "same", "reason", "foo.ts", "claude", nil, nil)
      local diffs = read_diffs(dir)
      assert.equals(1, #diffs)
      assert.is_nil(diffs[1].original)
      assert.equals("same", diffs[1].current)
      assert.equals("reason", diffs[1].reason)
    end)
  end)

  it("saves a note when original is nil (no recent fill) and reason is given", function()
    with_tmp_cwd(function(dir)
      learn.teach(nil, "some code", "use named functions", "foo.ts", "claude", nil, nil)
      local diffs = read_diffs(dir)
      assert.equals(1, #diffs)
      assert.is_nil(diffs[1].original)
      assert.equals("some code", diffs[1].current)
      assert.equals("use named functions", diffs[1].reason)
    end)
  end)

  it("does nothing when there is no diff AND no reason", function()
    with_tmp_cwd(function(dir)
      learn.teach("same", "same", "", "foo.ts", "claude", nil, nil)
      local diffs = read_diffs(dir)
      assert.is_nil(diffs)
    end)
  end)

...
describe("apply — error cases", function()
  ...
  it("returns false on unknown action", function()
    ...
    local ok, err = apply.apply({ file = path, action = "bogus", find = "hello", replace = "" })
    assert.is_false(ok)
...
describe("apply — delete action", function()
  it("deletes an existing file", function()
    local path = tmp_file("some content")
    local ok, err = apply.apply({ file = path, action = "delete", find = "", replace = "" })
    assert.is_true(ok)
    assert.is_nil(err)
  ...
  it("returns false when file does not exist", function()
    local ok, err = apply.apply({ file = "/nonexistent/delete_me.lua", action = "delete", find = "", replace = "" })
    assert.is_false(ok)
    assert.truthy(err:find("file not found"))
  end)

  it("closes open buffer for the deleted file", function()
    local path = tmp_file("buf content")
    local abs = vim.fn.fnamemodify(path, ":p")
    local bufnr = vim.fn.bufadd(abs)
    vim.fn.bufload(bufnr)
    assert.not_equals(-1, vim.fn.bufnr(abs))
    apply.apply({ file = path, action = "delete", find = "", replace = "" })
    assert.equals(-1, vim.fn.bufnr(abs))
  end)
end)
